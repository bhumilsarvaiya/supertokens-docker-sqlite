...
then
    ...
    # check if max server pool size is passed
    if [ ! -z $SQLITE_CONNECTION_POOL_SIZE ]
    then
        echo "sqlite_connection_pool_size: $SQLITE_CONNECTION_POOL_SIZE" >> $CONFIG_FILE
    fi

    # check if sqlite database name is passed
    if [ ! -z $SQLITE_DATABASE_NAME ]
    then
        echo "sqlite_database_name: \"$SQLITE_DATABASE_NAME\"" >> $CONFIG_FILE
    fi

    # check if sqlite key value table name is passed
    if [ ! -z $SQLITE_KEY_VALUE_TABLE_NAME ]
    then
        echo "sqlite_key_value_table_name: \"$SQLITE_KEY_VALUE_TABLE_NAME\"" >> $CONFIG_FILE
    fi

    # check if sqlite session info table name is passed
    if [ ! -z $SQLITE_SESSION_INFO_TABLE_NAME ]
    then
